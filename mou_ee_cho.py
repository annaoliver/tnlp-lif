...
def main_work():
    ...
    # C_m_I = c_i * nF
    # g_m_E = rand_seq(7.04,1.72,int(N_E))*0.1* nS
    # fs=rand_seq( 5.34,0.91,int(N_I/3))*0.1
    # bt=rand_seq(3.99,0.51,int(N_I/3))*0.1
    # mc=rand_seq( 2.98,0.55,int(N_I/3))*0.1
    ...
    for s_cho in cho_x:
        for echo in range(5):
            for para_ee in ee_:
                ...
                # plt.savefig("./mouse/ee/cho/%f-%d.jpg"%(para_ee,echo))
                # plt.close()
                ...
                np.savez('./mouse/ee/cho/ee%f-echo%d-s%f.npz'%(para_ee,echo,s_cho),**array_dict)
